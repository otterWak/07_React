...
const TodoList = () => {
  ...
  const handleAddTodo = () => {
    ...
    // ** 불변성을 유지해야 하는 이유
    //    : React가 상태 변경을 감지해야 하기 때문
    //      상태가 이전과 다르다고 React에게 알려주기 위해서는
    //      상택의 참조값(메모리 주소)이 기존과 다르게 변경되어야 함
    //      -> 변경되기 이전 원본의 상태값은 불변해야 하고,
    //         새로운 참조 객체를 만들어 이가 변경되었음을 React에게 알려줌
  ...
  const handleDeleteTodo = (index) => {
    // splice 함수 : mutates 함수(원본이 변경되는 함수)이므로 state인 todoList에 직접 사용 불가(불변성의 법칙)
    // todoList와 똑같은 배열 newTodoList를 만들어 splice이용 후, setState 함수 이용해서 상태 업데이트
    const newTodoList = [...todoList]; // todoList와 똑같은 배열 만들기 (복사)
    newTodoList.splice(index, 1); // index번호부터 1개 잘라내기
    setTodoList(newTodoList);
  }
