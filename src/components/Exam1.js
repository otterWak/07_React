...
class Exam1 extends Component {
  ...
  constructor(props) {
    ...
    // React 컴포넌트의 기본 초기화를 실행
    this.state = { count: 0 }; // state 객체 초기화
    // count 상태 값을 0으로 설정
    // this.state : 클래스형 컴포넌트에서 컴포넌트의 상태(state) 객체를 의미함
  }

  // 클래스형 컴포넌트 문법으로 정의된 메서드
  ...
  render() {
    return (
      <div>
        ...
        <br></br>
        <button onClick={this.downClick}>감소버튼</button>
      </div>
    );
  }
}

// 다른 파일에서 해당 컴포넌트를 사용하 수 있도록 내보냄
export default Exam1;
// Exma1 파일 안에, Exam1 파일 하나가 아닌 여러 개 만들 수 있음
// Exam1.js 파일에서 기본적으로 Exam1 컴포넌트를 내보내기 위해
// default 작성
